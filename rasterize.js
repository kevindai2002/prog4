...
var textureObjects = []; // this contains texture objects for each set
        ...
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    ...
                    var uvToAdd = inputTriangles[whichSet].uvs[whichSetVert]; // get uv to add
                ...
                gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array([255, 255, 255, 255])); // 1x1 white pixel
                var textureImage = new Image();
                textureImage.crossOrigin = "Anonymous";
                textureImage.src = INPUT_TRIANGLES_URL.substring(0, INPUT_TRIANGLES_URL.lastIndexOf('/') + 1) + inputTriangles[whichSet].material.texture;
                textureImage.onload = function() {
                    gl.bindTexture(gl.TEXTURE_2D, texture);
                    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, textureImage);
                    gl.generateMipmap(gl.TEXTURE_2D);
                };
                textureObjects[whichSet] = texture;
...
function setupShaders() {
    ...
    var vShaderCode = `
        ...
        varying vec2 vTextureUV; // interpolated uv for frag shader
    ...
    var fShaderCode = `
        ...
        varying vec2 vTextureUV; // uv of fragment

        void main(void) {

            // PART 2: texture only (no lighting)
            vec4 textureColor = texture2D(uTexture, vTextureUV);
            gl_FragColor = textureColor;

            /* PART 3 will use lighting + texture
            // ambient term
...
function renderModels() {
    ...
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
        ...
        // reflectivity: feed to the fragment shader (not used in Part 2, but kept for Part 3)
